...
name: init
description: >
  Set up a shared bridge folder for two or more project directories
---
...
- `/path/to/backend /path/to/frontend` — two project paths, default shared folder
- `/path/to/backend /path/to/frontend --share /path/to/share` — explicit shared folder
...
- Remind them to **restart Codex** in each project for the changes to take effect
- After restarting, they can use the register skill in each session to join the bridge
